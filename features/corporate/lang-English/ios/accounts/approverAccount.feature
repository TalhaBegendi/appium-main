@devices=pixel7
Feature: Accounts flows language Englısh - ANDROID- CORPORATE

  Background:
    Given Login as "CORPORATE" customer "NEW_ACCOUNT" using "ENGLISH" language
  ...
  Scenario: Accounts > Open Account - Turkish Lira
    When Go to "Accounts > Open Account" from Menu
    When Opens a "Checking Account" account with "Turkish Lira" currency
