@devices=pixel7
Feature: Saved Transactions Flows Language Turkish - ANDROID - CORPORATE

  Background:
    Given Login as "CORPORATE" customer "STANDARD_USER" using "TURKISH" language
    And Go to "Kayıtlı İşlemler" from Menu

  Scenario: CORPORATE - Kayıtlı Bir İşlem Üzerinden Para Transferi Talimatı Oluşturulması
  ...
  Scenario: CORPORATE - Hesaplarım Arasında Yeni Kayıtlı Transfer Oluşturulması
  ...
  Scenario: CORPORATE - Hesaplarım Arasındaki Kayıtlı Transferin Silinmesi
  ...
  Scenario: CORPORATE - IBAN’a Yeni Kayıtlı Transfer Oluşturulması
    When Click add new saved transaction button
    Then Verify add new saved transaction screen is visible
  ...
  Scenario: CORPORATE - IBAN’a Kayıtlı Transferin Silinmesi
  ...
  Scenario: CORPORATE - Hesap Numarasına Kayıtlı Transferin Silinmesi
    When Click delete button for saved transaction named "hesap numarası"
    Then Verify delete confirmation popup is displayed
    When Click confirm delete button on delete popup
    Then Verify saved transaction is deleted successfully

  Scenario: CORPORATE - Kayıtlı İşlemler Aramasında Eşleşen Sonuçların Listelenmesi
    When Into saved transactions search field
    Then All saved transactions matching displayed

  Scenario: CORPORATE - Kayıtlı İşlemler Aramasında Sonuç Bulunamaması
    When Enter "aa" into saved transactions search field
    Then No search results found message is displayed

  Scenario: CORPORATE - Kendi IBAN’ı Girildiğinde Uyarı Mesajının Gösterilmesi
    When Click add new saved transaction button
    Then Verify add new saved transaction screen is visible
    When Enter "iban" as new saved transaction name
    And Select "Para Transferi" as transaction type
